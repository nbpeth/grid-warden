...
app.delete("/api/v1/matrices/:id", (req, res) => {
    const { id } = req.params;
    postgresClient.deleteMatrixById(id)
    .then(() => {
...
app.put("/api/v1/matrices", (req, res) => {
    postgresClient.saveUserMatrix({...req.body})
    .then((r) => {
...
app.put("/api/v1/matrices/:id", (req, res) => {
    const { id } = req.params;
    postgresClient.saveUpdateUserMatrix({...req.body, id})
    .then((r) => {
        res.status(200).json(r?.[0]);
    })
})
...
app.get("/api/v1/matrices", (req, res) => {
    postgresClient.listUserMatrices({...req.body})
    .then((r) => {
...
app.get("/api/v1/matrices/:id", (req, res) => {
    const { id } = req.params;
    postgresClient.getMatrixById(id)
    .then((r) => {
        res.status(200).json(r);
    })
})
